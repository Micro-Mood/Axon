...
**错误**：`FILE_NOT_FOUND`、`ENCODING_ERROR`
...
| `content` | str | ✗ | `""` | 初始内容 |
| `encoding` | str | ✗ | `"utf-8"` | 写入编码 |
| `overwrite` | bool | ✗ | `false` | 文件存在时是否覆写 |
...
**错误**：`CONCURRENT_MODIFICATION`（文件存在且 overwrite=false）、`ENCODING_ERROR`
...
**错误**：`FILE_NOT_FOUND`、`INVALID_PARAMETER`（行号范围无效）、`ENCODING_ERROR`
...
**错误**：`FILE_NOT_FOUND`、`INVALID_PARAMETER`（行号无效）、`ENCODING_ERROR`
